...
const App = () => {
  ...
  useEffect(() => {
    ...
    setVisitorInfo(v => ({ ...v, device: /mobile|android|iphone|ipad/.test(ua) ? "Mobile" : "PC" }));
  }, []);
  ...
  useEffect(() => {
    const getGeo = async () => {
      try {
        ...
        let city = "";
        try { city = (await fetch(`https://ipapi.co/${ip}/json/`).then(r => r.json())).city || ""; } catch {}
  ...
  const toggleDarkMode = () => { setDarkMode(s => !s); setIsManualToggle(true); };

  const allCategories = ["全部", ...Object.keys(softwareData)];
  const filterSoftware = s => s.name.toLowerCase().includes(query.toLowerCase()) || s.description.toLowerCase().includes(query.toLowerCase());
  const filteredData = useMemo(() => {
    if (selectedCategory === "全部") return { 全部: Object.values(softwareData).flat().filter(filterSoftware) };
  ...
  return (
    <div className={darkMode ? "bg-gray-900 text-white min-h-screen" : "bg-gray-100 text-gray-900 min-h-screen"}>
      {/* Visitor Bar */}
      <div className={`w-full text-sm py-3 shadow-md transition-colors ${darkMode ? "bg-gradient-to-r from-gray-800 to-gray-700 text-gray-200 shadow-gray-900" : "bg-gradient-to-r from-blue-600 to-blue-500 text-white shadow-blue-300"}`}>
        <div className="max-w-6xl mx-auto px-4 flex flex-wrap justify-between gap-2 items-center">
          <span className="flex items-center gap-1">{visitorInfo.device === "Mobile" ? <Smartphone className="w-4 h-4"/> : <Monitor className="w-4 h-4"/>}{visitorInfo.device}</span>
          <span className="flex items-center gap-1"><Globe className="w-4 h-4"/>{visitorInfo.ip || "加载中..."}</span>
          <span className="flex items-center gap-1"><MapPin className="w-4 h-4"/>{visitorInfo.country || "未知"} {visitorInfo.city || ""}</span>
      ...
      <div className="flex justify-between items-center p-4 max-w-6xl mx-auto">
        ...
        <button onClick={e => { toggleDarkMode(); handleRipple(e); }} className="p-2 rounded-full bg-gray-200 dark:bg-gray-700 hover:scale-110 transition-transform relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 mb-6">
        <div className="relative w-full overflow-hidden rounded-2xl shadow-lg h-48 sm:h-64">
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
            {banners.map((_, i) => <span key={i} className={`w-2 h-2 rounded-full cursor-pointer ${currentBanner===i?"bg-white":"bg-gray-400"}`} onClick={() => setCurrentBanner(i)}></span>)}
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          {allCategories.map(cat => (
            <button key={cat} onClick={e=>{ setSelectedCategory(cat); handleRipple(e); }} className={`px-4 py-1.5 rounded-full text-sm font-medium relative overflow-hidden ${selectedCategory===cat?"bg-blue-600 text-white":"bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600"}`}>
              {cat}
            </button>
          ))}
        </div>
          ...
          Object.entries(filteredData).map(([category, softwares])=>(
            <div key={category}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {softwares.map((s,i)=>(
                  <div key={i} className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-md hover:shadow-xl transition relative">
                    ...
                    <div className="flex items-center justify-between mt-4">
                      ...
                      <a href={s.downloadUrl} target="_blank" rel="noopener noreferrer" onClick={handleRipple} className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 relative overflow-hidden">
